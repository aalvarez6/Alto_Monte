...
from physics import (
    ...
    HOUSEHOLDS, BESS_SOC_INIT, BESS_SOC_MAX, BESS_SOC_MIN
)
...
with st.sidebar:
    ...
    bess_cap = st.slider(
        "Capacidad BESS (kWh)",
        min_value=0, max_value=400, value=120, step=25,
...
with tab1:
    ...
    fig1.update_layout(
        ...
        plot_bgcolor="#0B1A12", paper_bgcolor="#0F2318",
...
with tab3:
    ...
    fig_bm.update_layout(
        barmode="group", plot_bgcolor="#0B1A12", paper_bgcolor="#0F2318",
        font=dict(family="JetBrains Mono", size=10),
...
with tab4:
    ...
    with col_dim1:
        h_dim = st.slider("Viviendas a cubrir", 0, 200, n_households, 10, key="dim_h")
